...
def _open_whisper(model_path, device, compute_type):
  model = WhisperModel(model_path, device=device, compute_type=compute_type)
  # CTranslate2 loads cuBLAS/cuDNN lazily, so a GPU whose CUDA libraries it
  # can't use (e.g. Colab's CUDA 13 runtime) only fails on first use.
  # Transcribe a second of silence to find out now, not mid-request.
  segments, _ = model.transcribe(np.zeros(16000, dtype=np.float32), language="en")
  list(segments)
  return model


def _open_on_best_device(model_path):
  """GPU first, then CPU: CPU is slower but works whatever the CUDA setup."""
  attempts = [("cuda", "float16"), ("cpu", "int8")] if torch.cuda.is_available() else [("cpu", "int8")]
  errors = []
  for device, compute_type in attempts:
    try:
      model = _open_whisper(model_path, device, compute_type)
      print(f"🗣️ Whisper ready on {device}")
      return model
    except Exception as e:
      errors.append(f"{device}: {e}")
      print(f"⚠️ Whisper couldn't run on {device} ({e})")
  raise RuntimeError("; ".join(errors))


def load_whisper_model(model_name=WHISPER_REPO):
  global whisper_model
  if whisper_model is None:
    try:
      whisper_model = _open_on_best_device(model_name)
    except Exception as e:
      print(f"⚠️ Loading {model_name} failed ({e}); downloading it directly ...")
      model_dir = download_model(model_name, download_folder="./", redownload=False)
      whisper_model = _open_on_best_device(model_dir)
